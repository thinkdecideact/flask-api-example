...
def get_count(conn, **kwargs):
    count_stmt = sa.select([sa.func.count('*')]).select_from(kwargs['table']).where(kwargs['where'])
...
def get_many(conn, **kwargs):
    ...
    limit_stmt = sa.select(kwargs['fields']).where(kwargs['where']).order_by(desc(kwargs['table'].c.mtime), desc(kwargs['table'].c.id)).limit(kwargs['row_count_per_page']).offset(kwargs['offset'])
...
def insert(conn, **kwargs):
    insert_stmt = sa.insert(kwargs['table']).values(kwargs['data'])
...
def update(conn, **kwargs):
    update_stmt = sa.update(kwargs['table']).where(kwargs['where']).values(kwargs['data'])
